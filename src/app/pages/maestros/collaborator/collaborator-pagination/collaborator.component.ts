...
export default class  TipoViaComponent {
  ...
  colDefs: ColDef[] = [
    { field: "idCollaborator", headerName :"Codigo", checkboxSelection: true, filter:true, width:100 },
  ...
  NewRegistter(){
    this.router.navigate(['collaborator-edit', '0'], { relativeTo: this.activeRouter.parent });
  }
  edit(){
    ...
    this.router.navigate(['collaborator-edit', id], { relativeTo: this.activeRouter.parent });
  }
  delete(){
    let rowData = this.gridApi.getSelectedRows();
    if(rowData.length == 0){
    ...
    .then(
      res => {
        if(res){
          ...
          this.crudService.deleteById("collaborator","",id)
            .subscribe(
              {
                next: (res) => {
