...
export default class  FunctionalLeaderComponent {
  ...
  colDefs: ColDef[] = [
    { field: "idFunctionalLeader", headerName :"Codigo", checkboxSelection: true, filter:true, },
  ...
  setDataSource(data:DatasourcePaginationInterface){
    ...
    const dataSource : IDatasource = {
      ...
      "getRows": (params:IGetRowsParams) => {
        ...
        this.paginationService.getPaginationAgGrid(this.currentPage, countPage, _filtroForBack, _sortForBack , "functionalLeader", "pagination")
        .subscribe( data =>{
  ...
  NewRegistter(){
    this.router.navigate(['functionalLeader-edit', '0'], { relativeTo: this.activeRouter.parent });
  }
  edit(){
    ...
    this.router.navigate(['functionalLeader-edit', id], { relativeTo: this.activeRouter.parent });
  }
  delete(){
    let rowData = this.gridApi.getSelectedRows();
    if(rowData.length == 0){
    ...
    .then(
      res => {
        if(res){
          ...
          this.crudService.deleteById("functionalLeader","",id)
            .subscribe(
              {
                next: (res) => {
