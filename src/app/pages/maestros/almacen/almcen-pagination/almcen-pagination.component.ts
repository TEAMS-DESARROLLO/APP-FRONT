...
import { AlmacenInterface } from '../almacen-interface';


@Component({
  selector: 'app-almcen-pagination',
  standalone: true,
  imports: [CommonModule, AgGridModule, ToolbarToolboxComponent],
  templateUrl: './almcen-pagination.component.html',
  styleUrl: './almcen-pagination.component.css'
})
export default class AlmcenPaginationComponent {
  ...
  colDefs: ColDef[] = [
    { field: "idAlmacen", headerName :"Codigo", checkboxSelection: true, filter:true, },
    { field: "dscAlmacen", headerName: "Descripcion", filter:true },
  ...
  private crudService = inject(CrudService<AlmacenInterface>);
  ...
  onGridReady(params: GridReadyEvent<AlmacenInterface>) {
  ...
  setDataSource(data:DatasourcePaginationInterface){
    ...
    const dataSource : IDatasource = {
      ...
      "getRows": (params:IGetRowsParams) => {
        ...
        this.paginationService.getPaginationAgGrid(this.currentPage, countPage, _filtroForBack, _sortForBack , "almacen", "pagination")
        .subscribe( data =>{
  ...
  NewRegistter(){
    this.router.navigate(['almacen-edit', '0'], { relativeTo: this.activeRouter.parent });
  }
  edit(){
    ...
    let id = rowData[0].idAlmacen;
    this.router.navigate(['almacen-edit', id], { relativeTo: this.activeRouter.parent });
  }
  delete(){
    let rowData = this.gridApi.getSelectedRows();
    if(rowData.length == 0){
    ...
    .then(
      res => {
        if(res){
          ...
          this.crudService.deleteById("almacen","",id)
            .subscribe(
              {
                next: (res) => {
