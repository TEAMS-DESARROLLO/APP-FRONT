...
@Component({
  ...
  styleUrl: './paquete-edit.component.css'
})
export default class PaqueteEditComponent {
  ...
  ngOnInit() {
    this.customForm = this.fb.group({
      idPaquete: ['0', Validators.required],
      dscPaquete: ['', Validators.required],
    ...
    this.activeRouter.params.subscribe(
      params => {
        ...
        if(id == "0"){
          ...
          this.crudService.readById("paquete","",id)
          .subscribe(
            data => {
  ...
  save(){
    ...
    this.crudService.create("paquete","create",data)
      .subscribe(
        res=> {
          this.customForm.reset() ;
          this.router.navigate(['paquete'], { relativeTo: this.activeRouter.parent });
  ...
  update(){
    ...
    let id = this.customForm.get('idPaquete')?.value;
    if(id){
      this.crudService.update('paquete','', data,id )
        .subscribe({
          next: (resp) => {
            this.customForm.reset() ;
