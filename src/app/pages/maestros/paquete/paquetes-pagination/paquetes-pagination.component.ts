...
import { PaqueteInterface } from '../paquete-interface';


@Component({
  selector: 'app-paquetes-pagination',
  standalone: true,
  imports: [CommonModule, AgGridModule, ToolbarToolboxComponent],
  templateUrl: './paquetes-pagination.component.html',
  styleUrl: './paquetes-pagination.component.css'
})
export default class PaquetesPaginationComponent {
  ...
  colDefs: ColDef[] = [
    { field: "idPaquete", headerName :"Codigo", checkboxSelection: true, filter:true, },
    { field: "dscPaquete", headerName: "Descripcion", filter:true },
  ...
  onGridReady(params: GridReadyEvent<PaqueteInterface>) {
  ...
  setDataSource(data:DatasourcePaginationInterface){
    ...
    const dataSource : IDatasource = {
      ...
      "getRows": (params:IGetRowsParams) => {
        ...
        this.paginationService.getPaginationAgGrid(this.currentPage, countPage, _filtroForBack, _sortForBack , "paquete", "pagination")
        .subscribe( data =>{
  ...
  NewRegistter(){
    this.router.navigate(['paquete-edit', '0'], { relativeTo: this.activeRouter.parent });
  }
  edit(){
    ...
    let id = rowData[0].idPaquete;
    this.router.navigate(['paquete-edit', id], { relativeTo: this.activeRouter.parent });
  }
  delete(){
    let rowData = this.gridApi.getSelectedRows();
    if(rowData.length == 0){
    ...
    .then(
      res => {
        if(res){
          ...
          this.crudService.deleteById("paquete","",id)
            .subscribe(
              {
                next: (res) => {
