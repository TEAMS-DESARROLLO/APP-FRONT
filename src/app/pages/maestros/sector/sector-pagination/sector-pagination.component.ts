...
@Component({
    selector: 'app-sector-pagination',
    standalone: true,
    templateUrl: './sector-pagination.component.html',
    styleUrl: './sector-pagination.component.css',
    imports: [CommonModule, AgGridModule, ToolbarToolboxComponent, ]
})

export default class SectorPaginationComponent {
  ...
  colDefs: ColDef[] = [
    { field: "idSector", headerName :"Codigo", checkboxSelection: true, filter:true, },
  ...
  onGridReady(params: GridReadyEvent<SectorInterface>) {
  ...
  setDataSource(data:DatasourcePaginationInterface){
    ...
    const dataSource : IDatasource = {
      ...
      "getRows": (params:IGetRowsParams) => {
        ...
        this.paginationService.getPaginationAgGrid(this.currentPage, countPage, _filtroForBack, _sortForBack , "sector", "pagination")
        .subscribe( data =>{
  ...
  NewRegistter(){
    this.router.navigate(['sector-edit', '0'], { relativeTo: this.activeRouter.parent });
  }
  edit(){
    ...
    let id = rowData[0].idSector;
    this.router.navigate(['sector-edit', id], { relativeTo: this.activeRouter.parent });
  }
  delete(){
    let rowData = this.gridApi.getSelectedRows();
    if(rowData.length == 0){
    ...
    .then(
      res => {
        if(res){
          ...
          this.crudService.deleteById("sector","",id)
            .subscribe(
              {
                next: (res) => {
