...
import { CommunityInterface } from '../community-pagination/community.interface';
...
@Component({
    ...
    styleUrl: './community-edit.component.css',
...
export default class TipoViaEditComponent implements OnExit {
    ...
    this.activeRouter.params.subscribe(
      params => {
        ...
        if(id == "0"){
          ...
          this.crudService.readById("community","",id)
          .subscribe(
            data => {
  ...
  save(){
    ...
    this.crudService.create("community","create",data)
      .subscribe(
        res=> {
          this.customerForm.reset() ;
          this.router.navigate(['community'], { relativeTo: this.activeRouter.parent });
  ...
  update(){
    ...
    let id = this.customerForm.get('idCommunity')?.value;
    if(id){
